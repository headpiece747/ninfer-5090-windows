        ...
        Json{{"device", engine_options.device},
             ...
             {"context_cache",
              Json{{"enabled", cache.enabled},
                   ...
                   {"policy",
                    cache.policy == ContextCachePolicy::Rolling ? "rolling" : "default"},
...
std::string format_throughput_json(const std::string& server_instance_id, std::uint64_t timestamp,
    ...
    record["context_cache"] = Json{
        {"captures", Json{{"completed", monotonic_delta(previous.active_captures_completed,
                          ...
                          {"offered", monotonic_delta(previous.active_captures_offered,
                                                      current.active_captures_offered)},
                          {"no_vacancy", monotonic_delta(previous.active_captures_no_vacancy,
                                                         current.active_captures_no_vacancy)},
                          {"plan_refused",
                           monotonic_delta(previous.active_captures_plan_refused,
                                           current.active_captures_plan_refused)},
                          {"infeasible", monotonic_delta(previous.active_captures_infeasible,
                                                         current.active_captures_infeasible)}}},
        {"selections",
         Json{{"root", monotonic_delta(previous.root_selections, current.root_selections)},
             ...
             {"shared_reuse_candidates",
              monotonic_delta(previous.shared_reuse_candidates, current.shared_reuse_candidates)},
             {"shared_reuse_declined",
              monotonic_delta(previous.shared_reuse_declined, current.shared_reuse_declined)},
             {"shared_reuse_key_mismatch",
              monotonic_delta(previous.shared_reuse_key_mismatch,
                              current.shared_reuse_key_mismatch)},
