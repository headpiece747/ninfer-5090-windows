...
> glider_find_unused_symbols. Build/test phases use the project's actual
> verification commands (see AGENTS.md): `dotnet build ModernWigiDash.slnx -c
> Release` and tests via `dotnet test` with temp `BaseOutputPath` to avoid a
> running app instance locking the App output.
