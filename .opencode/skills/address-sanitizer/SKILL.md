...
> **NInfer note:** this repo builds with MSVC 14.51 on Windows. The `clang`/`gcc` commands below
> are the reference form; on MSVC use `/fsanitize=address` and set `ASAN_OPTIONS` the same way.
> MSVC has no UBSan / TSan. The upstream skill itself marks Windows as limited-support, so prefer
> the `cuda-debugging` skill's `compute-sanitizer` for device-side faults. See the `sanitizers`
> skill for the whole sanitizer family and a chooser.
