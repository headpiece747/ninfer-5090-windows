...
> **NInfer note:** this repo builds with MSVC 14.51 on Windows. MSVC supports AddressSanitizer
> (`/fsanitize=address`) but not UBSan / TSan / MSan, so those workflows are advisory here; the
> `gcc`/`clang` flags below need translating to the MSVC equivalents. For device-side faults use
> the `cuda-debugging` skill's `compute-sanitizer`, and see `address-sanitizer` for the ASan
> deep-dive.
