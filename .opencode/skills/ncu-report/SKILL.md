...
> **NInfer overrides — these take precedence over the upstream defaults below:**
> - **Target hardware is an RTX 5090 (sm_120a, CC 12.0, 170 SMs, 32 GB GDDR7), not B200/sm_100.**
>   B200-only material (`reference/08-b200-metric-names.md`, the 148-SM/192 GB figures, parts of
>   `blackwell-cuda-programming.md`) is advisory. Enumerate real metric names with
>   `action.metric_names()` rather than trusting sm_100 names blindly.
> - **Run directory is `profiles/ncu/<run_name>/`**, matching this repo's documented layout
>   (`AGENTS.md` → Local operations; the repo uses `profiles/ncu/` and `profiles/nsys/`). The
>   reference docs below still say `profile/<run_name>/`; read that as `profiles/ncu/<run_name>/`,
>   and one directory per run still applies.
> - **Ignore `helpers/list_flashinfer_workloads.py` and the FlashInfer references** — this repo has
>   no FlashInfer workload dataset. `helpers/safetensors_loader.h` is still usable for real tensors.
> - The repo's own performance contract governs when this is worth invoking: measure at the claimed
>   scope, and profile only for an unresolved attribution (`AGENTS.md` → Verification).
> - Provenance: adapted from MIT HAN Lab's `ncu-report-skill` (MIT; `LICENSE` kept alongside).
...
**Target hardware (NInfer):** NVIDIA GeForce RTX 5090 (sm_120a, CC 12.0, 170 SMs, 32 GB GDDR7). The upstream skill targeted B200 (sm_100); the overrides above have precedence. Most advice below is generic; hardware-specific notes are explicitly marked.
...
0. **Create a new run directory first** under `profiles/ncu/<run_name>/` at the repo root — **one directory per run**, never reuse an existing one. Each run contains its own `harness/`, `reports/`, `analysis/`, and `REPORT.md`. This rule is mandatory in this repo. See [`reference/00-directory-layout.md`](reference/00-directory-layout.md).
...
2. **Build a standalone harness** unless the user is profiling through their existing binary. Harnesses compile in seconds, run the kernel in isolation, and let you use `-lineinfo` cleanly so ncu can map SASS back to source. Compile into `profiles/ncu/<run_name>/harness/`. See [`reference/02-harness-guide.md`](reference/02-harness-guide.md) and the template in [`helpers/harness_template.cu`](helpers/harness_template.cu).

3. **Run two profiles**: `--set full` (with `PmSampling` sections) for the overview, and `--set source --section SourceCounters` for per-line stall attribution. Write outputs to `profiles/ncu/<run_name>/reports/`. See [`reference/03-collection.md`](reference/03-collection.md).

4. **Parse with `ncu_report`** Python module — not by eye-balling the CLI. Write analysis outputs to `profiles/ncu/<run_name>/analysis/`. Use the helpers in [`helpers/`](helpers/). See [`reference/04-python-api.md`](reference/04-python-api.md).
...
7. **Write the report** at `profiles/ncu/<run_name>/REPORT.md` with evidence-backed recommendations, ranked by expected impact. See [`reference/07-report-template.md`](reference/07-report-template.md).
