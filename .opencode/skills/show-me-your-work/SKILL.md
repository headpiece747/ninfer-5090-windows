...
A single TSV file, one row per decision. TSV because it renders as a sortable table in the IDE, `column -t` and spreadsheets read it, and a row appends with one command. Cells stay single-line. Evidence is a pointer, not prose.
...
- **evidence.** A link or path that proves it: commit SHA, `file:line`, a test name with its run result, or an artifact/trace/screenshot path (a GliderTrace session id counts). Never a paragraph.
...
Use the helper so rows stay well-formed: `scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>` (when a POSIX shell is available). It stamps `ts`, writes the header on first use, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote so a reviewer opening the log in a spreadsheet doesn't trigger formula execution. Appending a single-line TSV row with the file tools works too; apply the same hygiene.
...
Commit it only when the work is ambitious enough that a reviewer needs the trail to trust the result: a large multi-phase migration, a hillclimb run, anything where confidence has to be shown rather than assumed.
...
At the end of the run, before handing back, check the log told the truth. Audit against what actually happened: the git history (`git log`), the build and test output, and any run artifacts the trail points at. Walk the log against reality:
...
Before handing back, spawn a fresh subagent (session model; the point is eyes that did not do the work) to review the trail. Self-review is not a substitute. The reviewer reads the audit trail and the run's evidence, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

- Decisions logged with weak or absent evidence.
- Verification steps skipped or claimed without proof in the run's output.
- Choices that look risky in hindsight (premature, scope-creeping, papering over a symptom).
- Gaps the user would otherwise miss on a casual skim.

Every reply for a run that produced a trail ends with an "Attention" section. Lead with "independent trail review", then list each flag pointing to specific rows or moments. "No flags" is a valid value. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
