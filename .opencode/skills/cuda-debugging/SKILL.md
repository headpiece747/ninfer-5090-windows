...
> **NInfer note:** target is an RTX 5090 (sm_120a) on Windows with CUDA 13.3 and MSVC 14.51.
> `compute-sanitizer` and the error-code triage apply directly — upstream #208 is a 700 /
> `cudaErrorIllegalAddress` on NVFP4+MTP. `cuda-gdb` has limited Windows support; use a Linux
> host when interactive GPU stepping is required. The `--arch=sm_80` in the examples is
> illustrative; build with the repo's configured `sm_120a`. See `ncu-report` for performance
> analysis once correctness is established.
